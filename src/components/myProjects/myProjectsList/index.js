import NetflixProject from "../../images/projects/netflixProject.png";
import SpotifyProject from "../../images/projects/spotifyProject.png";
import LinkedinProject from "../../images/projects/linkedinProject.png";

export const MyProjectsList = [
  {
    id: 1,
    title: "Netflix Clone - React.js + API + Netlify",
    img: NetflixProject,
    imgTag: "Netflix project",
    link: "https://gj-netlix-clone.netlify.app/",
  },

  {
    id: 2,
    title: "Linkedin Clone - React.js + NoSQL Firebase",
    img: LinkedinProject,
    imgTag: "Linkedin project",
    link: "https://gj-linkedin-clone.netlify.app/",
  },

  {
    id: 3,
    title: "Spotify Clone - React.js + API + Netlify",
    img: SpotifyProject,
    imgTag: "Spotify project",
    link: "https://gj-spotify-clone.netlify.app/",
  },
];
